# 設定の形式

regied の設定ファイルのスキーマ。この形にした理由は
[ADR 0002](../adr/0002-configuration-schema.md)、kind ごとのフィールドは
[`kinds.md`](kinds.md)、実例は [`config/example.yaml`](../../../config/example.yaml) にある。

英語が正である。[docs/spec/configuration.md](../../spec/configuration.md) を参照。

> **状態: スキーマは形が決まった段階で、細部は確定していない。** 以下の kind と
> 持ち分は決定である。個々のフィールド名は最初のリリースまでに動くことがあり、
> `v1alpha1` はその意味である。

## 文書

ホスト 1 台につき YAML 1 文書。

| フィールド | 必須 | 値 |
|---|---|---|
| `apiVersion` | はい | `net.unstable.cloud/v1alpha1` |
| `kind` | はい | `NetworkConfig` |
| `metadata.name` | はい | このホストの設定に付ける名前。ホスト名ではない |
| `spec.global` | いいえ | ホスト全体のスイッチ。後述 |
| `spec.resources` | はい | リソースの一覧 |

apiGroup にプロジェクト名を入れないのは意図的である。バイナリを改名しても
設定ファイルが無効になってはならない。

**文書の kind にも役割名は入れない。** ファイアウォールだけのホストも、回線を 2 本
終端するホストと同じ `NetworkConfig` を書く。そのホストが何をするかは、どのリソースを
並べたかが表すのであって、文書の名前が表すのではない。必須のリソース kind は 1 つも無く、
`router` や `firewall` は型の側に現れない（[ADR 0009](../adr/0009-ownership-boundary.md)）。

`spec.resources` の各要素は次を持つ。

| フィールド | 必須 | 値 |
|---|---|---|
| `kind` | はい | [`kinds.md`](kinds.md) の 12 kind のいずれか |
| `metadata.name` | はい | kind の中で一意。他のリソースから参照される |
| `spec` | はい | kind ごとに異なる |

**一覧の順序に意味は無い。** 評価順が意味を持つところ（ポリシールーティングと、
1 つの政策の中のルール）は、明示の `priority` フィールドか内側のリストの順序で表す。
`spec.resources` の順序では表さない。

## リソース間の参照

リソースは `<kind の lower camel case>Ref` というフィールドで相手を参照し、
値は相手の `metadata.name` である。kind はフィールド側で決まるので値には書かない。

| フィールド | 指す先 |
|---|---|
| `interfaceRef`、`underlayRef` | `Interface` |
| `egressRef` | 回線 —— `PPPoESession` または `DSLiteTunnel` |
| `linkRefs` | リンクリソースの一覧 —— `Interface` / `PPPoESession` / `DSLiteTunnel` |
| `from`、`to` | `FirewallZone`、または予約名 `self` |
| `addressSetRefs` | `IPAddressSet` の一覧 |

以下 2 つの総称を使う。**リンクリソース**とは、ホストにリンクを作る kind
（`Interface` / `PPPoESession` / `DSLiteTunnel`）のことである。**回線**とは
`PPPoESession` または `DSLiteTunnel`、すなわち外に向かうリンクリソースであり、
NAT とポリシールーティングが指せるのはこれだけである。

存在しないリソースへの参照は検証エラー。参照の循環も同じ。

## ホスト全体のスイッチ: `spec.global`

カーネルの設定と、ファイアウォール全体に効く挙動が 1 つ。これらはリソースではない。
ホストにつき 1 つずつしかなく、誰も参照しない。

| フィールド | 既定 | 効果 | バックエンド |
|---|---|---|---|
| `ipForwarding` | `false` | インターフェース間で IPv4 / IPv6 を転送する | カーネル |
| `synCookies` | `true` | SYN flood 対策 | カーネル |
| `logMartians` | `false` | ありえない送信元アドレスのパケットをログする | カーネル |
| `sendRedirects` | `false` | ICMP redirect を送る | カーネル |
| `receiveRedirects` | `false` | ICMP redirect を受け入れる | カーネル |
| `sourceValidation` | `false` | リバースパスフィルタ | カーネル |
| `mssClamp` | `auto` | 手前のセグメントより MTU の小さい経路で TCP MSS を抑える | nftables |

2 つ注記する。

**`sourceValidation` の既定は無効。** ポリシールーティングは設計として戻りの経路を
非対称にする。経路表が選ばないインターフェースに応答が返ってくるのであり、
strict なリバースパスフィルタはまさにそれを落とす。有効にしてよいのは
`EgressRoutePolicy` が 1 つも無いホストだけで、regied はこの組み合わせを拒否する。
値は `all`、`default`、そして設定が名指すリンクのうちその時点でホストに在るものの
それぞれに書く。カーネルは `all` とリンク自身の設定の大きい方を適用するからである。

**`mssClamp: auto`** は、インターフェースの種別を 1 つ名指すのではなく、
手前のセグメントより MTU が低いすべての経路で抑える。トンネルは PPPoE と同じくらい
これを必要とする。`off` で無効、数値を書くとその値に固定する。

カーネルの設定は適用時に反映し、適用が失敗したときに戻せるよう記録する。
**`/etc/sysctl.d/` には書かない。** そこに置くと起動時に反映され、
ファイアウォールがまだ無いうちに転送が有効になるからである。

## どこに落ちるか

| 領域 | kind | バックエンド |
|---|---|---|
| リンク、アドレス、MTU、ブリッジ | `Interface` | systemd-networkd |
| 静的ルート | `Interface` | systemd-networkd |
| プレフィックス委譲、ルーター広告 | `Interface` | systemd-networkd |
| IPv4 over IPv6 のトンネル | `DSLiteTunnel` | systemd-networkd |
| ルーティングポリシールールとそのテーブル | `EgressRoutePolicy` | systemd-networkd |
| PPPoE の回線 | `PPPoESession` | pppd |
| 発呼したアドレスを、ファイアウォールの回線セットに入れること | `PPPoESession` | pppd の ip-up / ip-down フック |
| アドレス配布、DNS | `DHCPServer`、`DNSForwarder` | dnsmasq |
| ホスト自身のリゾルバ（`listenOn` に `loopback` があるとき） | `DNSForwarder` | systemd-networkd。systemd-resolved が読む |
| ファイアウォール、NAT、ポリシールーティングの判定 | `FirewallZone`、`FirewallPolicy`、`IPAddressSet`、`SourceNAT`、`PortForward`、`EgressRoutePolicy` | nftables |
| 回線のアドレスを持つ DNS レコード | `DNSRecordSet` | 事業者の HTTP API |
| カーネルのスイッチ | `spec.global` | カーネル |

regied は networkd の設定を自分の接頭辞のファイルとして `/etc/systemd/network/` に置き、
すべての `DHCPServer` と `DNSForwarder` から 1 つの dnsmasq 設定を組み立て、
nftables は自分のテーブルだけを作り替える。`PPPoESession` を宣言したホストには
スクリプトが 2 つ増える。`/etc/ppp/ip-up.d/regied-uplink-set` と
`/etc/ppp/ip-down.d/regied-uplink-set` である。ルールセットは回線のアドレスを持たないので、
発呼したアドレスを hairpin のルールがマッチするセットに入れるのがこの 2 つである
（[ADR 0015](../adr/0015-uplink-addresses-in-sets.md)）。セッションが 1 つも無くなれば
回収する。これらのディレクトリはどれもディストリビューションと共有なので、regied が置くものは
自分の名前と所有の印を持つ。ルールセット全体を flush することは無く、
自分が作っていないファイルを書き換えることも無い
（[ADR 0009](../adr/0009-ownership-boundary.md)）。

これとは別に、regied 自身の状態ディレクトリの下に 3 つのファイルがある。regied の外の
誰も読まないし、どれも人が書く設定ではない。

| ファイル | 何であるか |
|---|---|
| 受け入れられた宣言 | このホストが収束していく先の宣言。提出されたときのバイト列そのもの。**再起動をまたいで生き残らなければならないのはこれだけである** |
| 最後の周回の報告 | 最後の周回がホストをどこに残したか。診断であって 2 つめの宣言ではない。何もそこへは収束しない |
| 適用済みのルールセット | 最後に入れた nftables のテキスト。次の周回が比較の起点にする |

受け入れられた宣言に秘密情報は入っておらず、入りようもない。このスキーマに秘密情報を
入れられるフィールドが無いからである（[ADR 0003](../adr/0003-secrets-out-of-configuration.md)）。
リビジョンはそのバイト列のダイジェストなので、同じファイルの写しを持っている者なら
regied 無しで同じ値を計算できる。

## 宣言がホストに届くまで

> **この節は [ADR 0017](../adr/0017-submission-through-the-resident-process.md) により
> 実装された提出経路を記述する。**

**ディスク上のファイルは、ホストが収束していく先ではない。** `regied apply` がそれを読んで
常駐プロセスへ送り、常駐プロセスが検証してステージし、検証とステージングを通った時点で、
このホストが受け入れた宣言として書き出す。その瞬間から効いている宣言はその記録であり、
収束するものはファイルではなく記録を読む
（[ADR 0016](../adr/0016-converging-on-the-accepted-declaration.md)）。ホストを書くのは
常駐プロセスだけである（[ADR 0017](../adr/0017-submission-through-the-resident-process.md)）。

| コマンド | 読むもの | すること |
|---|---|---|
| `regied render` | 設定ファイルだけ。どのホストも読まない | 設定が何を意味するかを印字する |
| `regied apply` | 設定ファイル | 宣言を常駐プロセスへ送る。常駐プロセスがそれを記録し、そこへ向けて 1 周回す。周回がホストをどこに残したかを印字する。常駐プロセスが動いていなければ拒まれる |
| `regied apply --confirm duration` | 設定ファイル | 宣言を期限つきの試行として送る。受け入れ済み宣言は置き換えない |
| `regied apply --dry-run` | 設定ファイルとこのホスト | 適用すれば何が変わるかを印字し、何も変えない。常駐プロセスは要らない |
| `regied serve [-resync duration] [-control path]` | 記録とこのホスト | 常駐プロセス。制御ソケットで提出と試行を受け付け、再同期（既定 1 分）とアドレスイベントで収束させ、確認試行を保持する |
| `regied confirm` | ローカル制御ソケット | 有効な試行を受け入れ済み宣言へ書き、リビジョンと最後の周回の状態を返す |
| `regied cancel` | ローカル制御ソケット | 有効な試行を捨て、受け入れ済み宣言への提出権限を持つ周回を直ちに走らせる |

**設定ファイルを読むのは `regied apply` だけである。** `regied apply` と `regied render` は、
`--config` で別のパスを指さない限り `/etc/regied/config.yaml` を読む。`serve` は設定ファイルの
フラグを持たず、読むのは記録とクライアントが送ってきたものだけで、netlink イベントは完全な比較を
早めるだけである。書きかけの編集がホストの設定になるのを防いでいるのはこれであり、自分で走るものは
ファイルに触れることすらできない。

提出は自分の周回を待ち、常駐プロセスが報告に書くのと同じ内容を印字する。リビジョン、状態、
フェーズごとに何が変わったか、何を待っているか、何がどこで失敗したか。周回は接続ではなく
常駐プロセスのものなので、`apply` を中断しても、ssh のセッションが落ちても、周回は最後まで
走り、報告がどう終わったかを言う。

記録が持つのは**求められたもの**であって、最後にうまくいったものではない。コマンドが失敗した
提出は、新しい宣言を記録に残す。ホストは失敗が残した場所に居て、報告がそう言う。
前の宣言へ戻ることは、前のファイルを適用することであり、普通の提出である。記録された宣言
そのものも、そうしたファイルの 1 つである。

制御ソケットの既定パスは `/run/regied/control.sock` である。ソケットは root のものである。
そこに届くことはホストを設定し直す能力そのものなので、パッケージは他の誰にも与えない。
受け付けるメッセージは、宣言の提出、試行の開始または置換、確認、取消の 4 つだけである。
これは宣言がホストへ向かう経路であって、読み取り API ではない。答えるのはメッセージが起こした
周回についてだけで、status、報告、dry-run、render、reload、リソース操作は運ばない。

試行が有効な間、常駐ループの各周回は試行を spec として使う。期限切れと取消は同じ経路を
通り、メモリ上の試行を捨てて、受け入れ済み宣言への提出権限を持つ周回を直ちに走らせる。
daemon の停止や再起動でも試行は消える。永続する宣言は置き換わっていないため、次の周回は
安全な向きへ動く。試行中の普通の `apply` は受け入れ済み宣言を置き換え、その場で試行を終える。
最後の周回の報告には診断価値があるため、有効な試行とそのリビジョン・期限を載せるが、
報告は引き続き診断であり 2 つめの spec にはならない。

ユニットは `regied.service` の 1 つであり、どの起動の後でも常駐プロセスの最初の周回が、
再起動の後にルールセットも他のすべても戻すものになる。その周回は無人の周回である。起動した
ばかりのホストでは何も上がっていないので無人の周回の制約は何も縛らず、単に再起動されただけの
ホストでは、デーモンの再起動がセッションの再起動になるのを防ぐ。デーモンを止めれば収束が
止まり、ホストには何も起きない。無効にすれば、起動時の収束も手放すことになる。

## 周回がどこで終わるか

どの周回も 3 つの状態のいずれかで終わり、どれであるかを言う。

| 状態 | 意味 |
|---|---|
| `converged`（収束済み） | ホストは宣言をまるごと持っている。省いたものも再試行中のものも無い |
| `waiting`（待機） | できることはすべて行い、適用時にしか存在しない値が無いために何かを省いている。何を待っているかを名指す |
| `failing`（失敗） | 試して駄目だった。あるいは記録がこの版の regied の受け付けるものではない。何が失敗したかを名指す |

終了ステータスは状態に従う。`failing` は非 0、`waiting` と `converged` は 0 である。
待機は起動したばかりのホストの普段の状態であって、提出の欠陥ではない。

**周回が省くのは、無い値に依存する成果物まるごとである。** その小さい版ではない。そして
前の周回が書いたものは回収せずそのまま残す。`aftrHost` がまだ解決していない `DSLiteTunnel` は
2 つのファイルのどちらも書かれず、`duidFile` を読めなかった `Interface` は自分のファイルを
持たない。DUID 無しで書いたプレフィックス委譲は、宣言されたものの小さい版ではなく、
**委譲されるプレフィックスが黙って変わる設定**なので、まったく書かない。

認証情報だけは例外で、何かが走る前に周回を止める。認証情報なしに回線を上げることは
劣化した成功ではないし、認証情報のファイルに小さい版は無い。API トークンはこの仲間では
ない。それを使う 1 つの手順が読むものであり、読めなかったトークンは、戻すべき
ファイアウォールを抱えた周回を止めるのではなく、そのレコードを *失敗中* にする。

DNS レコードは、失敗しても周回をまったく止めない唯一のものである。カーネルが既に持って
いるアドレスから最後に書かれ、周回の他の何もそれに依存しないので、届かない事業者は
そのレコードを自分だけのバックオフの下で失敗中にするだけで、周回の残りは最後まで進む
（[ADR 0019](../adr/0019-uplink-address-in-dns-records.md)）。

次の 2 つのホストは、何も変えずそのままにして、そう伝える。宣言を一度も受け入れていない
ホストと、記録がこの版の regied ではもう受け付けられないホストである。空へ収束することは
ファイルが 1 つ無いことを理由に動いているルーターからファイアウォールを外すことであり、
半分しか理解できていない宣言へ収束することは、収束しないことより悪い。

## 秘密情報

**このスキーマに秘密情報を入れられるフィールドは無い。** 認証情報は、それを収めた
ファイルのパスで名指し、そのファイルは設定の外に置く
（[ADR 0003](../adr/0003-secrets-out-of-configuration.md)）。PPPoE のユーザー ID も、
`DNSRecordSet` の API トークンも認証情報として扱う。参照先が無い・読めない場合は
検証エラーになる。

認証情報は、それを必要とする周回が読んで捨てる。PPPoE のファイルは毎回の周回で読む。
セッションのオプションファイルをそこから描画するからである。API トークンを読むのは
レコードを書く周回だけなので、レコードがすべてあるべき場所にあるホストはトークンを
1 度も読まないし、dry run は決して読まない。

## 導出される値

利用者が手で整合を取らねばならないものを、いくつか導出に変えてある。

| 導出されるもの | 元 | 固定できるか |
|---|---|---|
| ルーティングテーブル番号 | `EgressRoutePolicy` | できる（`spec.table`） |
| ファイアウォールマーク | `EgressRoutePolicy` | できる（`spec.mark`） |
| 政策のテーブルに入るデフォルトルート | 政策の `egressRef` | できない |
| マークからテーブルへのルーティングポリシールール | 政策 | できない |
| ポートフォワードのためのファイアウォールの穴 | `PortForward` | できる（`spec.openFirewall: false`） |
| ポートフォワードが公開されている回線とファミリの組ごとのルーティングテーブルとファイアウォールマーク | `PortForward` | できない |
| そのテーブルに入るデフォルトルート、それを選ぶルーティングポリシールール、転送された接続にマークを保存し戻りで復元するルール | `PortForward` | できない |
| 政策の先頭に置く状態追跡のルール | `FirewallPolicy` | できる（`spec.stateful: false`） |

導出された番号は `regied render` で見える。**regied の外からこれに依存してはならない。**
判定が経路になるまでの実装の都合でしかない。

テーブル番号は 100 から、マークは 0x100 から順に割り当てる。割り当ては政策が評価される
順序、つまりファミリ・`priority` の順で走り、**文書がリソースを並べた順では走らない。**
だからファイル内でリソースを動かしても番号は変わらず、何も変えていない適用は何も変えない。
固定された値はそのまま残し、割り当てはそれを避けて進む。カーネルが自分用に持っている
`main` / `local` / `default` のテーブルは、固定値としても拒否する。政策の後ろに、
ポートフォワードが公開されている回線が、ファミリ・名前の順で続く。そのテーブルとマークは
固定できない（[ADR 0020](../adr/0020-port-forward-reply-on-arrival-uplink.md)）。

この 2 つはあくまで割り当ての開始点であり、個々の政策が何番を取るかの約束ではない。
上の一文はそのまま生きている。

## 回線 2 本のホストで、部品がどう噛み合うか

設定はトラフィックについての判断を表し、その判断は 2 つの半分に分かれてカーネルに届く。

1. `EgressRoutePolicy` は**どのトラフィックか**を書く（送信元のレンジと、
   除外すべき宛先）。この半分は照合であり、マークを打つ nftables ルールになる
2. 同じリソースが**どの回線か**を名指す。この半分は、その回線へのデフォルトルートを
   持つルーティングテーブルと、マークでそのテーブルを選ぶルーティングポリシールールになる

**マークを打つチェーンは nat prerouting より後ろで走る。** この順序が hairpin を
成立させる。LAN 側の端末が回線のグローバルアドレス経由で公開サービスに繋ぐとき、
マークを考える時点で宛先は既に内側のホストに書き換わっている。だから政策の
ローカル宛除外に当たり、ローカルに配送される。**回線のグローバルアドレスを
どこにも書き留める必要が無い。** このスキーマにそれを受け取るフィールドが無いのは
そのためで、`PortForward` と `SourceNAT` は `egressRef` を取る。

3. `PortForward` が 3 つ目の部品を足す。回線に届いた接続の戻りは、その回線から出ていく。
   フォワードが書き換えた最初のパケットで回線のマークを接続に保存し、reply 方向の
   パケットでは政策を見る前にそれを復元する。だから変換先自身のトラフィックをもう一方の
   回線に送る政策があっても、戻りには触れない。そのために回線は自分のテーブルとマークを
   持ち、政策と同じように導出される
   （[ADR 0020](../adr/0020-port-forward-reply-on-arrival-uplink.md)）。

## 検証

ここに書くのは提出が確かめることであり、何かを記録する前・走らせる前に行う。記録に対する
周回も同じことを確かめるが、宣言が名指すファイルだけは別で、それらを読むのは周回自身である。
読めなかったときに何が起きるかは上に書いた。その後で消えたファイルが、この版の regied が
受け付けない宣言のように見えてはならない。

参照が解けること・必須フィールドがあることに加えて、regied は次を拒否する。

- `sourceValidation: true` と `EgressRoutePolicy` の同居
- 同じファミリで `priority` が重複する `EgressRoutePolicy`
- `from` と `to` が同じ `FirewallPolicy` が 2 つ
- `ifname` が他のインターフェースの `bridge.members` に現れ、かつアドレスも持つ `Interface`
- 上流インターフェースに PD クライアントが無いのに、委譲プレフィックスから導く指定
- `egressRef` が `DSLiteTunnel` を指す `PortForward` / `SourceNAT`
  （AFTR 側で変換されるので、内向きに公開できない）
- 待ち受けるレンジと変換先が同じ数のポートを覆っていない `PortForward`
- `protocol` が `tcp` / `udp` の名前以外である `PortForward`
- `sourceRanges` に CIDR でも閉区間でもない裸のアドレスを書いた `EgressRoutePolicy`
- `aftrHost` と `aftrAddress` の両方を持つ `DSLiteTunnel`
- そのどちらも持たない `DSLiteTunnel`
- ホストのリンク名になる名前が 15 文字を超えるもの。カーネルが保持できるのはそこまでである。
  対象は `Interface` の `ifname` と `bridge.members`、`PPPoESession` と `DSLiteTunnel` の
  `metadata.name`
- 2 つ目の `DNSForwarder`。ホストを 1 つの dnsmasq が受け持ち、
  キャッシュも上位リゾルバも 1 組しか無い
- `self` という名前の `FirewallZone`
- `from` が `self` である `FirewallPolicy`
- `provider` が `cloudflare` 以外である `DNSRecordSet`。実装があるのはその 1 つだけである
- `type` が `AAAA` である `DNSRecordSet` のレコード。ここのレコードが追従するのは
  このホストの回線が持つアドレスであり、IPv6 のレコードが代わりに何を持つべきかは
  まだ決めていない
- `egressRef` が `DSLiteTunnel` を名指す `DNSRecordSet` のレコード。
  そこからは何も公開できず、レコードが持つべきアドレスも無い
- リソースの `zone` の中に無い名前を挙げている `DNSRecordSet` のレコード
- 同じ名前と型を求める `DNSRecordSet` の項目が 2 つあること。1 つのリソースの中でも、
  2 つのリソースにまたがっていても同じで、レコードがどちらを持つのかがどこにも書かれていない
- `ttl` が 1 分未満または 24 時間を超える持続時間である `DNSRecordSet` のレコード。
  および `proxied: true` のレコードに持続時間を書いたもの。その TTL は事業者のものである
- 参照先の秘密情報ファイルが無い・読めない・空

次は警告にとどめ、適用は続ける。

- `dhcpv6.prefixDelegation` を持ちながら `duidFile` が無い `Interface`。
  新規に引く回線には持ち越す DUID が無く、それは正当な構成である。
  既に委譲を受けている機器を入れ替える構成でこれを省くと、
  **黙って別のプレフィックスを引く。**
- `ipv6` を持つが、そのインターフェースが `otherInformation` を広告していない
  `DHCPServer`。そこに書いた内容を誰も聞きに来ない。
