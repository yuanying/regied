...
> **状態: 開発中。** まだ動かせるものは何も無い。設定スキーマも確定していない。
...
**スキーマは暫定である。** 例は「こういう形になる」を示すためのもので、
そのまま使うためのものではない。スキーマが固まったら差し替える。

秘密情報は設定に書かない。PPPoE のパスワードのような認証情報は、
リポジトリの外にあるファイルへのパスとして参照する。
...
道具が揃った環境なら `make test-netns` で直接走る。前提が欠けているときは
理由を述べて飛ばすが、コンテナ経由の実行では飛ばさずに失敗する。
